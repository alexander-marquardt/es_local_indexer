...
Keep in mind that this application has not been specially tuned or configured for presenting the Elasticsearch documentation. Therefore the search results may not be exactly as one might expect - for example, if you search for the words "ingest nodes", you will received what several results which appear to be nearly identical. This is because we have ingested all of the documentation for all versions of Elasticsearch, and we have not added any filtering for a specific version and each release may have nearly identical documentation. If this application were targetted only for presenting Elasticsearch documentation, we would likely add a "version" dropdown along with a [bool filter](https://www.elastic.co/guide/en/elasticsearch/reference/7.3/query-dsl-bool-query.html) to ensure that only documentation from a single version of Elasticsearch is presented for any given search. 
